...
class CardEventSetAssigneeTests extends GrailsUnitTestCase {
  ...
  void testSetAndUnsetAssignee() {
    ...
    def username = "assignee"
    def userRealname = "New Assignee"
    def email = "mr.krister@mail.com"
    def passwd ="pass"
    def userEventCreate = new UserEventCreate(username: username, userRealName: userRealname, email: email, enabled: true, passwd: passwd, passwdRepeat: passwd)
    userEventCreate.authenticateService = authMock.createMock()

    userEventCreate.beforeInsert()
    if( userEventCreate.save() ){
      userEventCreate.process()
    }

    def setAssigneeEvent = new CardEventSetAssignee(
            card: card1onPhase1,
            assigneeDomainId: userEventCreate.eventCreator.domainId,
            eventCreator: userEventCreate.eventCreator
    )
    ...
    assertEquals userEventCreate.eventCreator, card1onPhase1.assignee

    def unsetAssigneeEvent = new CardEventSetAssignee(
            card: card1onPhase1,eventCreator: user2
    )
